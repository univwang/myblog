---
title: go-libp2p学习（二）
date: 2023-06-08 16:52:18
tags: [Go, libp2p]
excerpt:  HTTP proxy service with libp2p
categories: Go
index_img: /img/index_img/19.png
banner_img: /img/banner_img/background20.jpg
---
...
设置host的流处理协议和函数streamHandler，返回一个结构体
```go
...
构建一个buf读取数据，当stream到来时，使用http读取请求请求。修改URL信息，便于发送，使用http.DefaultTransport发送请求，将返回数据write到stream中
...
### Serve
拨号并监听http请求，其中p需要实现ServeHTTP，处理http的回复和发送
...
1. 构建远端主机节点
2. 创建NewProxyService，并阻塞
3. 构建本地主机节点
4. 连接远端主机节点
5. 拨号并监听端口http请求，此时发送到的指定端口的http请求会由本地节点处理，通过p2pstream向远端发送，远端处理stream流，将http应答返回，本地主机再将应答返回
